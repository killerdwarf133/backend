...
/**
 * Seeds the database. Assumes a live Mongo connection (the caller connects).
 *
 * - `force: false` (default) is idempotent — it seeds ONLY when the database is
 *   empty, so it's safe to call on every server start.
 * - `force: true` wipes every collection first and re-seeds (the `npm run seed`
 *   reset; destructive — never run on a live database with real data).
 */
export async function runSeed({ force = false }: { force?: boolean } = {}): Promise<{ seeded: boolean }> {
  const existing = await Team.countDocuments();
  if (existing > 0 && !force) {
    console.log(`Seed: database already initialised (${existing} teams) — skipping.`);
    return { seeded: false };
  }
